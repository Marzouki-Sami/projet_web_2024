...
try{
    ...
    $sth = $dbco->prepare("SELECT * FROM petition where user_id in (SELECT user_id FROM utilisateur WHERE user_profession = :profession and user_mail = :mail)");
    $sth->bindValue(':profession', "prof");
    ...
    if(empty($resultat)){
        ...
        echo "<h3>Les Pétitions Des Enseignant</h3>";
        echo "<br>";
        ...
        foreach($resultat as $row){
            ...
            echo "<td><a class='btn btn-secondary btn-block' href='voirpetmodif.php?id=".$row['petition_id']."'>Modifier</a></td>";
            echo "</tr>";
            $i++;
        }
        echo "</table><br>";
